...
xstudy <- read.table("C:/Users/colev/Dropbox/aMNLFA_12212017/newdata_06102021/ordinal/ordinal.dat", header = TRUE, na = ".")
names(xstudy) <- c("ID",  paste0("ORD_", 1:12), "AGE", "GENDER", "STUDY")

input.object <- aMNLFA.object(dir = "C:/Users/colev/Dropbox/aMNLFA_12212017/newdata_06102021/ordinal", 
                              mrdata = xstudy, 
                              indicators = paste0("ORD_", 1:12), 
                              catindicators = paste0("ORD_", 1:12), 
                              meanimpact = c("AGE", "GENDER", "STUDY"), 
                              varimpact = c("AGE", "GENDER", "STUDY"), 
                              measinvar = c("AGE", "GENDER", "STUDY"), 
                              factors = c("GENDER", "STUDY"), 
...
aMNLFA.prune<-function(input.object){
  ...
  round2estimates$pexact <- 2*(1 - pnorm(abs(round2estimates$est_se)))
  ...
  if (thresholds == TRUE) {
    ...
    intdif <- intdif[(grepl(".ON", intdif$paramHeader) == TRUE) & (grepl("ETA", intdif$paramHeader) == FALSE),]
    }
  ...
  if (thresholds == TRUE) {
    prune.summary <- list("Mean Impact" = meanimpact, "Variance Impact" = varimpact, "Loading DIF" = lambdadif, "Threshold DIF - Highest Category Used" = intdif, "Threshold DIF - All Categories Used" = tdif)
  } else {
    prune.summary <- list("Mean Impact" = meanimpact, "Variance Impact" = varimpact, "Loading DIF" = lambdadif, "Intercept DIF" = intdif)
  }
