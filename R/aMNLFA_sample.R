...
aMNLFA.sample<-function(input.object){
  ...
  if (!is.na(mytime)) write.table(srdata, paste(path,"/srdata.dat",sep=""),quote=FALSE, sep="\t",col.names=TRUE,row.names=FALSE)
  if (is.na(mytime)) write.table(mruse, paste(path,"/srdata.dat",sep=""),quote=FALSE, sep="\t",col.names=TRUE,row.names=FALSE)
