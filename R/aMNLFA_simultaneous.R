...
aMNLFA.simultaneous <- function(input.object){
  ...
  #START WITH LOADINGS BECAUSE INTERCEPTS DEPEND ON THEM
  
  dif <- list()
  l.design <- matrix(NA, l, length(myindicators))
  l.string.new <- list()
  l.string.eq <- list()
  ...
  for (ll in 1:l) {
    ...
    l.input <- l.fun(ll, dif[[ll]], myindicators, .05)
...
if (thresholds = TRUE) {
  ...
  diflist <- unique(uniquethreshold, uniquelambda)
  ...
  con <- unique(append(keepvarimpact, uniqueloading))
